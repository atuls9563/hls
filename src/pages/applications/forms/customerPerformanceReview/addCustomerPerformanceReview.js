...
import styles from '../../../../../styles/addCustomerPerformanceReview.module.css'
import { useTranslation } from 'react-i18next'
import Link from 'next/link'
import Breadcrumbs from '@mui/material/Breadcrumbs'

const AddCustomerPerformanceReview = () => {
    ...
    <div>
      <Breadcrumbs className={styles.breadcrumb}>
        ...
        <Link href='/applications/forms/customerPerformanceReview/addCustomerPerformanceReview'>
          <Typography className={styles.link} variant='subtitle'>{`${t(
            'Add Customer Performance Review / KPI / QBR'
          )}`}</Typography>
        </Link>
      </Breadcrumbs>
        ...
        <Grid container className={styles.container} justifyContent='space-between'>
          <Grid item xs={12} md={6}>
            <Typography variant='h5' className={styles.topHeading}>
              {`${t('Add Customer Performance Review / KPI / QBR')}`}
            </Typography>
          </Grid>
          <Grid item xs={12} md={12}>
            <Typography variant='subtitle2' className={styles.asterTwo}>
              {`${t(
                'Note: Current global site(Heliaus Test Site V2 (1106661) - #1106661) is used for the Customer Performance Review / KPI / QBR.'
              )}`}
            </Typography>
          </Grid>
...
export default AddCustomerPerformanceReview
